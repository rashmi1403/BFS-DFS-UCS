...
public class Breadth_First_Search 
{
	...
	public void bfs() throws IOException
	{
		...
		//While queue is not empty - Follow the BFS algorithm
		while(!pqueue.isEmpty())
		{
			...
			{
				goal_fnd = 1;
				// Goal reached so break out of the while
				break;
			}
			else
			{
		...
		//Print output if goal is reached 
		if(goal_fnd == 1)
		{
		...
		for( ; ; )
		{
			...
			for(f=1;f <= p;f++)
			{
				if(track_parent[f][1].equals(child))
				{
					...
					rev_path.push(track_parent[g][0]);
